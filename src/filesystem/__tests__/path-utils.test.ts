...
describe("Path Utilities", () => {
  describe("convertToWindowsPath", () => {
    ...
    it("never converts WSL paths (they work correctly in WSL with Node.js fs)", () => {
      ...
      expect(convertToWindowsPath("/mnt/c/NS/MyKindleContent")).toBe(
        "/mnt/c/NS/MyKindleContent"
      );
      expect(convertToWindowsPath("/mnt/d/Documents")).toBe("/mnt/d/Documents");
    });
    ...
    it("handles drive letter paths based on platform", () => {
      ...
      expect(convertToWindowsPath("/mnt/d/some/path")).toBe("/mnt/d/some/path");
  ...
  describe("normalizePath", () => {
    ...
    it("always preserves WSL paths (they work correctly in WSL)", () => {
      ...
      expect(normalizePath("/mnt/d/Documents")).toBe("/mnt/d/Documents");
    });
    ...
    it("handles special characters in paths", () => {
      ...
      expect(normalizePath("C:/NS/Sub&Folder")).toBe("C:\\NS\\Sub&Folder");
      // WSL paths should always be preserved
      expect(normalizePath("/mnt/c/NS/Sub&Folder")).toBe(
        "/mnt/c/NS/Sub&Folder"
      );
    ...
    it("capitalizes lowercase drive letters for Windows paths", () => {
      ...
      // WSL paths should always be preserved
      expect(normalizePath("/mnt/d/my/folder")).toBe("/mnt/d/my/folder");

      if (process.platform === "win32") {
        // On Windows, Unix-style paths should be converted and capitalized
        expect(normalizePath("/e/another/folder")).toBe("E:\\another\\folder");
      } else {
  ...
  describe("WSL path handling (issue #2795 fix)", () => {
    ...
    it("should NEVER convert WSL paths - they work correctly in WSL with Node.js fs", () => {
      ...
      expect(normalizePath("/mnt/c/Users/username/folder")).toBe(
        "/mnt/c/Users/username/folder"
      );

      expect(normalizePath("/mnt/d/Documents/project")).toBe(
        "/mnt/d/Documents/project"
      );
    });
    ...
    it("should convert Unix-style Windows paths (/c/) only when running on Windows (win32)", () => {
      ...
      expect(normalizePath("/c/Users/username/folder")).toBe(
        "C:\\Users\\username\\folder"
      );

      expect(normalizePath("/d/Documents/project")).toBe(
        "D:\\Documents\\project"
      );
    });
    ...
    it("should preserve regular Unix paths on all platforms", () => {
      ...
      expect(normalizePath("/home/user/documents")).toBe(
        "/home/user/documents"
      );
